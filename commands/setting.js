...
exports.run = async (client, message, args) => {
   if (message.guild.id !== `864845158079397928`) return;
...
// 	}
	...
	await axios.get(`${config.firebase_url}/guilds/${message.guild.id}.json`)
		.then(function (response) {
	...
	if (setup == false){
		...
		// fetch data
		await axios.get(`https://api.roblox.com/groups/${args[0]}`)
			.then(function (response) {
				groupID = Number(args[0]);
		...
		for (i = 0; i < roles.length; i++){
          ...
          .setFooter(`${copy}`)
          .setTimestamp()
					.setDescription(`**:white_check_mark: Rank: ${roles[i].Name} | Required ${config.experience_name}: 0 :white_check_mark: **\n\nThe first rank is automatically set to 0 ${config.experience_name}`)
          ...
          .setFooter(`${copy}`)
          .setTimestamp()
					.setDescription(`**:white_check_mark: Rank: ${roles[i].Name} | Required ${config.experience_name}: -1 :white_check_mark: **\n\nThe last rank, ${roles[i].Name}, is not reachable via promotions`)
			...
			const location = await message.author.send(  {embed: {
				...
				color: 16747520,
				description: `How many ${config.experience_name} points should be required to achieve the rank of **\`${roles[i].Name}\`** (roleset id: ${roles[i].Rank})?\n\n**If you'd like this rank to not be reachable through ${config.experience_name}, chat \`-1\`**`,
			}})
          ...
          .setTimestamp()
          .setFooter(`${copy}`)
					.setDescription(`**:white_check_mark: Rank: ${roles[i].Name} | Required ${config.experience_name}: ${Number(responseArray[0])} :white_check_mark: **`)
		...
		await client.channels.cache.get('705507172133634138').send(doneEmbed);
		
		var doneEmbed = new Discord.MessageEmbed()
			.setColor("GREEN")
			.setTitle(`**Ranks & ${config.experience_name} Requirements**`)
		...
		for (i = 0; i < numbers.length; i++){
			if (numbers[i] == -1){
				...
				doneEmbed.addField(`**\`${roles[i].Name}\` - \`(${roles[i].Rank})\`**`, `**${Number(numbers[i])} ${config.experience_name}**`, true);
			}
		}
		...
		return message.channel.send(`**If you plan on changing a setting, you must \`.unbind\` then rebind \`.bind ${args[0]}\`!**`)
	}else{
